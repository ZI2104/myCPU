...
>
> 每页均提供：**页面内容**、**讲稿文案**、**图表建议**，可直接转成 PPT 文案。
...
### 收尾关键词
> "不是堆功能，而是构建一套**架构可解释、实现可验证、结果可复现**的课程级模拟器平台。"
...
```
NEMU  启示 → 优先做好"可验证性闭环"（DiffTest + 调试器 + 快照）
QEMU  启示 → 调试接口要"可集成"到真实工具链（GDB RSP 完整性）
Spike 启示 → 语义正确性需要体系化回归（分层对比 + 失败复盘）
```
...
对标调研的结论也并入了选型：NEMU 提醒我们优先做好可验证性闭环；QEMU 提醒我们调试接口要能集成进真实工具链；Spike 强调语义正确性需要体系化回归。所以这个项目从一开始就不是单纯堆功能，而是以"可验证、可调试、可回归"为主线推进。
...
地址空间上，RAM 在 0x0 开始，CLINT 在 0x0200_0000，UART 在 0x1000_1000，NPU/GPU 模拟加速器在 0x2000_0000 区段。

---

**【P4 · 流水线，约 50 秒】**

流水线是标准的 **6 级**：pre-IF（预取指）、IF（取指）、ID（译码）、EX（执行）、MEM（内存访问）、WB（写回）。比经典 5 级多了一个 pre-IF 阶段，用来发起指令内存读请求，解决同步 RAM 的 1 周期延迟。
...
项目按课设要求的第 4–8 周推进，6 个阶段全部完成。**386 个测试全部通过**，实现了 40+ 条 RV32I 指令（加 M 扩展乘除法）、完整特权级与中断系统、Sv32 MMU 虚拟内存、GDB 调试服务器、DiffTest 框架，以及 HPM 性能监控。

每个阶段都有对应的验收证据：Phase 1-2 是单元测试；Phase 3 是流水线回归；Phase 4-5 是 DiffTest 与 OS 启动；Phase 6 是 xv6 shell 命令矩阵 5/5 全通过，以及 `run_phase6_showcase_pipeline.ps1` 一键演示脚本全链路 PASS。
