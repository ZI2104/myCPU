...
| Phase 6 | ✅ 完成 | 性能监控 (HPM CSR + PerfCollector) |

> 课设周次口径：本项目进度统一映射在第4-8周；本文件中的“预计周期”仅表示功能相对工作量，不再对应历史绝对周次编号。
...
```
课设结束后：Sv32 虚拟内存增强
后续拓展：简单 OS 内核 (基于 rCore-Tutorial)
长期拓展：NPU/ML 扩展 + MNIST demo
```
