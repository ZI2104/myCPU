...
| Phase 4 | Linux 用户态 SDL/FB 游戏演示                               | 🟡 部分完成 | 简化 2D 游戏跑通 + 输入设备 + Overlay                 | Framebuffer 可视化链路已完成；输入外设（MMIO+WebSocket+前端面板）已接通；游戏流程与 Overlay 仍未完成                                                                             |
