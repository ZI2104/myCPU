...
| ------- | ---------------------------------------------------------- | ---------- | ---------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| Phase 0 | 冻结可用基线 + OS bring-up 专项测试入口                    | 🟡 部分完成 | xv6 到 shell prompt 作为首个里程碑 | 基线（RV32I+特权/中断/可视化）已可用；xv6 shell 尚未达成                                                                           |
| Phase 1 | xv6 启动关键能力补齐（RV32M、Trap闭环、Sv32、MMU统一路径） | ✅ 已完成   | xv6 内核入口与串口输出稳定         | RV32M/Trap/Sv32/MMU 路径均已实现并有测试，详见本文件 P1/P3 验收段                                                                  |
| Phase 2 | xv6 可交互运行（CLINT/PLIC 稳定 + 最小块设备）             | 🟡 部分完成 | xv6 文件系统镜像进入 shell 可交互  | CLINT/PLIC 基础实现已就绪；VirtIO-Block 已补齐最小 desc 边界校验并覆盖 OUT→IN 路径，且 CLI 已支持预载磁盘镜像，镜像/交互链路待完成 |

> 最新进展（2026-04-01）：已补齐 PLIC 的 S 态窗口（`SENABLE/SPRIORITY/SCLAIM`）与 `SIP` 的机器态 `SSIP` 置位语义，并将 CPU 中断判定升级为“先 M 后 S”；在优化 PLIC pending 扫描后，40M 窗口吞吐恢复到约 `6.30 MIPS`，且 1.2B 长窗口稳定通过（无 panic）。当前可观测到 `sip=0x2` 曾生效，但 `virtio_irq/uart_irq/meip` 仍长期为 0，尚未进入 shell。 
